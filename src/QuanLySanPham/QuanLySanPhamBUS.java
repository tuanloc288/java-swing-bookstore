
package QuanLySanPham;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.List;
import java.util.Map;

public class QuanLySanPhamBUS extends mainBUS_DAO.mainBUS{
    public ArrayList<SanPham> dssp = new ArrayList<>();
    QuanLySanPhamDAO qlspDAO = new QuanLySanPhamDAO();
    
    public QuanLySanPhamBUS(){
        this.DAO = new QuanLySanPhamDAO();
        dssp = qlspDAO.readDB();
    }
    
    public List<String> getColumns(){
        List<String> column = new LinkedList<>();
        column.add("id");
        column.add("name");
        column.add("author");
        column.add("size");
        column.add("category");
        column.add("coverType");
        column.add("datePublic");
        column.add("detail");
        column.add("discount");
        column.add("url");
        column.add("stock");
        column.add("issuers");
        column.add("pages");
        column.add("price");
        column.add("supplier");
        column.add("translator");
        return column;
    }
    
    public int Add(List<Object> data , List<String> column){
        int flag = qlspDAO.insertData(data, column);
        readDB();
        return flag;
    }
    
    public int Update(List<Object> dataSet , List<String> updateColumns , List<String> conditionColumns){
        int flag = qlspDAO.updateData(dataSet, updateColumns, conditionColumns);
        readDB();
        return flag;
    }
    
    public int Delete(List<Object> conditionData, List<String> conditionColumns){
        int flag = qlspDAO.deleteData(conditionData, conditionColumns);
        readDB();
        return flag;
    }
    
    public String getSingleData(List<Object> dataSet, List<String> dataColumns, List<String> selectedColumns, boolean distinct) {
        String searchData = "";
        Map<String, List<Object>> rs = new HashMap<>();
        int temp = qlspDAO.selectData(dataSet, dataColumns, selectedColumns, rs, distinct);
        if(temp == 1){
            if(!rs.isEmpty()){
                for(String key : rs.keySet()){
                    List<Object> records = rs.get(key);
                    for(Object record: records){
                        searchData = String.valueOf(record);
                    }
                }
            }
            return searchData;
        }
        else if(temp == 2){
            return "2";
        }
        else return "0";
    }
    
    public ArrayList<SanPham> getDSSP(){
        return this.dssp;
    }
    
    public void readDB(){
        this.dssp.clear();
        this.dssp = qlspDAO.readDB(); 
    }
}
